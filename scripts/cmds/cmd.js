...
module.exports = {
	...
	onStart: async ({ args, message, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, event, commandName, getLang }) => {
		...
		// ==== Special UID Check ====
		// FIX: SPECIAL_UID is an array, so it must be checked with
		// .includes() — comparing a string to an array with !== is always
		// true regardless of contents, which meant NOBODY (not even the
		// special UIDs) could ever pass this check before.
		if (!SPECIAL_UID.includes(String(event.senderID))) {
	...
	onReaction: async function ({ Reaction, message, event, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang }) {
		...
		// ==== Special UID Check ====
		// FIX 1: event.senderID is the sender of the ORIGINAL message (the bot's
		...
		// FIX 2: SPECIAL_UID is an array — must use .includes(), not !==,
		// or this check is always true and blocks everyone.
		if (!SPECIAL_UID.includes(String(event.userID))) return;
